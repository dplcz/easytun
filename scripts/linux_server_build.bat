...
for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "$tags = docker images --format '{{.Tag}}' %BIN_NAME% | Where-Object { $_ -match '^v\d+\.\d+' }; if ($tags) { $latest = $tags | Sort-Object { [version]($_ -replace '^v','') } -Descending | Select-Object -First 1; $parts = ($latest -replace '^v','').Split('.'); $parts[-1] = [int]$parts[-1] + 1; Write-Output ('v' + ($parts -join '.')) } else { Write-Output 'v1.0' }"`) do set NEW_TAG=%%i
